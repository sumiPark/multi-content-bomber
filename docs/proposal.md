...
다음 두 메뉴는 사이드바에 `soon` 뱃지로 노출되며, 본 사양 범위에서는 안내 페이지만 제공한다.

- **AI DM 자동 응답 :** OpenAI GPT-4o가 인스타그램·틱톡·유튜브 DM의 1차 응대를 자동 작성하고, 키워드 분기·사람 핸드오프 큐를 제공. 캡션·브랜드 톤을 학습 소스로 사용.
- **예약 규칙 (Recurring Rules) :** 요일·시간대 반복 배포, 콘텐츠 태그 → 계정 그룹 자동 매칭 규칙, Smart Scheduler 추천 슬롯의 템플릿화.

### 3.5 정보 구조 (Information Architecture)

사이드바는 사용자 행동 기준으로 4개 영역으로 분류한다. 상세는 `docs/functional-specification.md §9`.

- **최상단 (자주 보는 화면) :** 대시보드, 분석
- **콘텐츠 :** 콘텐츠 생성 (5-Step 마법사), 배포 관리, 보관함
- **자동화 (Phase 2~) :** DM 자동 응답, 예약 규칙
- **설정 (한 번 세팅) :** 계정 연동, AI 프리셋
